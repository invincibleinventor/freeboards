...
export default function Home() {
 ...
 const handleLinkOpen = () => setLinkOpen(!open);
 

  const error = false
  async function checklink(link:any){
    const {data,error:err} = await supabase.from('boards').select('*').eq('boardname',link)
    if(error){
      console.log(error)
    }
    ...
    if(data && data.length>0){
      ...
      onPasswordChange('')
      setOpen(true)
      alert('Link Already Taken')
    }
    else{
      onLinkTrue(true)
      await supabase.from('boards').insert({boardname:link,password:password,content:'# SAMPLE CONTENT'}).then((e)=>window.location.replace('/boards/'+link))
  ...
  return (
   <div className='h-screen flex xl:py-20 py-5 px-5 md:px-0 flex-col items-center content-center'>
    ...
    <section className="grow py-12 md:py-20 max-w-[78rem] mx-auto px-4 sm:px-6">
      ...
      <Button className="my-12 content-center mx-auto flex items-center gap-3" onClick={()=>handleLinkOpen()}>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
      ...
      >
        <DialogHeader>Create New Bulletin</DialogHeader>
        <DialogBody>
          <div className="flex flex-col items-center content-center mx-auto space-y-4">
        <Input
        type="text"
        size='lg'
        value={link}
        placeholder="Enter a bulletin name"
        className="border font-inter !text-[16px]  !w-72 border-gray-300 bg-white text-gray-900 shadow-lg shadow-gray-700/5 ring-4 ring-transparent placeholder:text-gray-800 placeholder:font-medium  !font-inter  placeholder:!font-inter focus:border-gray-700 focus:!border-t-gray-700 focus:ring-gray-700/10"
        labelProps={{
          className: "hidden",
        }}
      ...
      />
        ...
        <DialogFooter>
          ...
          <Button variant="gradient" color="green" onClick={()=>checklink(link)}>
            <span>Ok!</span>
          </Button>
        </DialogFooter>
      </Dialog>
